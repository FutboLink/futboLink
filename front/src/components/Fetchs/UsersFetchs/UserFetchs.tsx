...
export const fetchRegisterUser = async (user: IRegisterUser) => {
  try {
    ...
    console.log(
      "REGISTRO - Datos del usuario a enviar:",
    ...
    if (dataToSend.trayectorias && Array.isArray(dataToSend.trayectorias)) {
      ...
      const formattedTrayectorias = dataToSend.trayectorias
        ...
        .map((exp) => ({
          ...
          // Liga en la que compitió (la que el usuario seleccionó).
...
export const getCv = async (cvPath: string) => {
  try {
    ...
    if (cvPath.match(/^https?:\/\/(res\.)?cloudinary\.com/)) {
      ...
      if (cvPath.endsWith(".pdf")) {
        return {
          type: "pdf",
          directUrl: cvPath,
          googleViewerUrl: `https://docs.google.com/viewer?url=${encodeURIComponent(
            cvPath
          )}&embedded=true`,
          downloadUrl: `${cvPath.replace(/\.pdf$/, "")}/fl_attachment/${cvPath
            .split("/")
            .pop()}`,
        };
      }
    ...
    if (
      cvPath.includes("cloudinary.com") ||
      cvPath.includes("res.cloudinary.com")
    ) {
      ...
      if (fullUrl.endsWith(".pdf")) {
        return {
          ...
          downloadUrl: `${fullUrl.replace(/\.pdf$/, "")}/fl_attachment/${fullUrl
            .split("/")
            .pop()}`,
        };
      }
    ...
    if (cvPath.includes("upload/") || cvPath.includes("/pdf/")) {
      ...
      const cloudinaryUrl = `https://res.cloudinary.com/dagcofbhm/${
        cvPath.startsWith("/") ? cvPath.substring(1) : cvPath
      }`;
      ...
      if (cloudinaryUrl.endsWith(".pdf")) {
        return {
          ...
          downloadUrl: `${cloudinaryUrl.replace(
            /\.pdf$/,
            ""
          )}/fl_attachment/${cloudinaryUrl.split("/").pop()}`,
        };
      }
    ...
    try {
      ...
      console.log(
        "Intentando obtener CV a través del endpoint con filename:",
        filename
      );
      ...
      if (cvPath.includes("v1") || cvPath.includes("upload")) {
        ...
        console.log(
          "Intentando URL directa de Cloudinary:",
          cloudinaryDirectUrl
        );
        ...
        if (cloudinaryDirectUrl.endsWith(".pdf") || cvPath.includes("/pdf/")) {
          return {
            ...
            downloadUrl: `${cloudinaryDirectUrl.replace(
              /\.pdf$/,
              ""
            )}/fl_attachment/${cloudinaryDirectUrl.split("/").pop()}`,
          };
        }
