...
import { FaArrowLeft, FaThumbsUp, FaBookmark, FaShare, FaEllipsisV } from 'react-icons/fa';
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={isOwnProfile ? 'Gestiona tu perfil en FutboLink' : `Perfil de ${profile.name} ${profile.lastname} - ${profile.puesto || ''}`} />
      </Head>
                ...
                {profile.trayectorias && profile.trayectorias.length > 0 && (
                  <div className="bg-white rounded-lg p-4 shadow-md border border-gray-200 mb-4">
                    <div className="flex items-center">
                      ...
                      <div>
                        <h3 className="font-bold text-gray-800">{profile.trayectorias[0].club}</h3>
                        <div className="flex items-center text-sm text-gray-600">
                          ...
                          <span>{profile.trayectorias[0].nivelCompetencia || 'Primera división'}</span>
                        </div>
                      </div>
                      <div className="ml-auto">
                        <button className="text-green-600 text-sm font-medium">Ver más</button>
                      </div>
                    </div>
                    <div className="mt-3 text-sm text-gray-600">
                      <span>Hasta {profile.trayectorias[0].fechaFinalizacion ? new Date(profile.trayectorias[0].fechaFinalizacion).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' }) : 'No especificada'}</span>
                    </div>
                  </div>
                )}
