...
/**
 * Script simple para exportar usuarios de la base de datos de Render
 * 
 * Este script requiere que proporciones la URL de la base de datos de Render como argumento:
 * node export-users-simple.js "postgres://usuario:contraseña@host:puerto/nombre_db"
 */
...
if (!databaseUrl) {
  ...
  console.error('Ejemplo: node export-users-simple.js "postgres://usuario:contraseña@host:puerto/nombre_db"');
  process.exit(1);
}

// Función principal
async function exportUsers(dbUrl) {
  ...
  try {
    ...
    const outputFile = path.join(exportsDir, `users-simple-${timestamp}.json`);
    ...
    // Crear archivo CSV
    const csvFile = path.join(exportsDir, `users-simple-${timestamp}.csv`);
...
exportUsers(databaseUrl).catch(err => {
