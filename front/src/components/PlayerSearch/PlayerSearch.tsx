...
const PlayerSearch: React.FC = () => {
  ...
  const renderUserCard = (currentUser: User) => {
    ...
    // Obtener el nombre del ultimo Club del Usuario
    const trayectorias = currentUser?.trayectorias as
      | { club: string }[]
      | undefined;
    ...
    return (
      <div
        key={currentUser.id}
        className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all relative"
      >
        ...
        <div className="px-4 pb-4 pt-12 relative">
          ...
          <div className="absolute -top-10 left-4">
            <div className="relative">
              ...
              <div
                className={`absolute -right-1 -bottom-1 ${badgeBgClass} text-white text-xs py-0.5 px-2 rounded-full font-medium shadow-sm`}
              >
                <span>{badgeText}</span>
              </div>
            </div>
