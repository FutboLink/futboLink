...
export class EmailService {
  ...
  constructor(
    ...
    if (!host || !port || !user || !pass) {
      this.logger.error('Email configuration is incomplete. Check environment variables.');
      this.logger.error(`MAIL_HOST: ${host ? 'Set' : 'Not set'}`);
      this.logger.error(`MAIL_PORT: ${port ? 'Set' : 'Not set'}`);
      this.logger.error(`MAIL_USER: ${user ? 'Set' : 'Not set'}`);
      this.logger.error(`MAIL_PASSWORD: ${pass ? 'Set' : 'Not set (or empty)'}`);
    ...
    this.transporter.verify((error) => {
      if (error) {
        ...
        this.logger.error('If using Gmail, make sure you are using an App Password, not your regular password.');
        this.logger.error('Generate an App Password at: https://myaccount.google.com/apppasswords');
        this.isConfigValid = false;
      } else {
  ...
  async sendEmail(options: EmailOptions): Promise<boolean> {
    try {
      // Si la configuración no es válida, registrar un error pero no intentar enviar
      if (!this.isConfigValid) {
        this.logger.error('Email service is not properly configured. Email will not be sent.');
        this.logger.error('Please check your email configuration in the .env file.');
        // En entorno de desarrollo, simular éxito para no interrumpir el flujo
        if (process.env.NODE_ENV === 'development') {
          this.logger.warn('DEV MODE: Simulating successful email send');
          return true;
        }
      ...
      if (error.message.includes('Invalid login') || error.message.includes('Username and Password not accepted')) {
        this.logger.error('Authentication failed. If using Gmail, make sure you are using an App Password.');
        this.logger.error('Generate an App Password at: https://myaccount.google.com/apppasswords');
      } else if (error.code === 'ECONNREFUSED') {
        this.logger.error(`Could not connect to mail server at ${this.configService.get<string>('MAIL_HOST')}:${this.configService.get<number>('MAIL_PORT')}`);
      }
      
      // En entorno de desarrollo, podemos simular éxito para no interrumpir el flujo
      if (process.env.NODE_ENV === 'development') {
        this.logger.warn('DEV MODE: Simulating successful email send despite error');
        return true;
      }
